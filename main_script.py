...
# --------------- AI Content Control Function ---------------

def should_use_ai_content(excel_path):
    """
    Check the 'Do you want AI Content?' setting from the Summary sheet.
    Returns True if the value is 'Yes', False otherwise.
    """
    try:
        kv = read_summary_keys(excel_path, sheet_name="Summary")
        ai_setting = kv.get("Do you want AI Content?", "").strip().lower()
        return ai_setting == "yes" and client is not None
    except Exception as e:
        print(f"Warning: Could not read AI content setting: {e}")
        return False  # Default to False if there's an error

# --------------- helpers ---------------

def read_summary_keys(excel_path, sheet_name="Summary"):
    """Read key-value pairs from Summary sheet"""
...
def generate_overview_ai_content(excel_path, existing_kv=None, use_ai=True):
    """Generate detailed overview content using AI"""
    ...
    try:
        ...
        Write a comprehensive, technical, and market-focused overview.
        """
...
def generate_market_overview_content(excel_path, existing_kv=None, use_ai=True):
    """Generate market overview content using AI"""
    if not use_ai or not client:
        return ""
    ...
    try:
        ...
        prompt = f"""
        Write a detailed market overview for the {kv.get('Title', '')} market in exactly 500 words. Use paragraph form.
        ...
        Focus on comprehensive market intelligence for business decision-making.
        """
...
def get_sheet_percentage_data(sheet_name, workbook):
    """Extract percentage data from segmentation sheets"""
...
def debug_process_sheet_percentage_data(sheet_name, wb, latest_year=2024):
    """
    Debug version to see exactly what data is being extracted
    """
    ...
    for row_idx in range(1, 20):
        ...
        if "(%" in row_text and str(latest_year) in row_text:
            print(f"Found percentage header at row {row_idx}: {row_text}")
            for col_idx, cell in enumerate(row):
                if cell and str(latest_year) in str(cell) and "(%" in str(cell):
                    header_row_idx = row_idx
                    year_col_idx = col_idx
                    print(f"Year {latest_year} found at column {col_idx}: {cell}")
                    break
            break
    
    if header_row_idx is None or year_col_idx is None:
        print(f"Could not find percentage section for {latest_year}")
        return [], []
    
    # Extract percentage data
    data = []
    print(f"Extracting data from rows after {header_row_idx}:")
    
    for row_idx in range(header_row_idx + 1, min(header_row_idx + 10, ws.max_row + 1)):
        row = list(ws.iter_rows(min_row=row_idx, max_row=row_idx, values_only=True))[0]
        
        if not row[0] or "Total" in str(row[0]):
            print(f"Row {row_idx}: Stopping at empty or Total row")
            break
        ...
        try:
            ...
            if 0 < value < 1:
                value = round(value * 100, 1)
                print(f"Row {row_idx}: {item_name} = {raw_value} (decimal) -> {value}%")
            elif value:
                value = round(value, 1)
                print(f"Row {row_idx}: {item_name} = {raw_value} -> {value}%")
            else:
                print(f"Row {row_idx}: {item_name} = {raw_value} (zero/null)")
                
            data.append((item_name, value))
        except (ValueError, TypeError, IndexError) as e:
            print(f"Row {row_idx}: Error processing {item_name}: {e}")
            continue
    
    data.sort(key=lambda x: x[1] if x[1] is not None else 0, reverse=True)
    print(f"Final sorted data: {data}")
    return data, data

def process_sheet_percentage_data_enhanced(sheet_name, wb, top_n=None, latest_year=2024):
    """
    Enhanced version that better handles different percentage data formats
    """
    if sheet_name not in wb.sheetnames:
        return [], []
    ...
    # Strategy 1: Look for explicit percentage header with year
    header_row_idx = None
    year_col_idx = None
    
    for row_idx in range(1, 25):  # Search more rows
        row = list(ws.iter_rows(min_row=row_idx, max_row=row_idx, values_only=True))[0]
        row_text = " ".join(str(cell) if cell else "" for cell in row)
    ...
    # Strategy 2: If strategy 1 fails, look for "Volume Share" or similar
    if header_row_idx is None:
        for row_idx in range(1, 25):
            row = list(ws.iter_rows(min_row=row_idx, max_row=row_idx, values_only=True))[0]
            row_text = " ".join(str(cell) if cell else "" for cell in row).lower()
            
            if ("volume share" in row_text or "market share" in row_text) and str(latest_year) in row_text:
                for col_idx, cell in enumerate(row):
    ...
    # Strategy 3: Look for any row with the year that comes after volume data
    if header_row_idx is None:
        volume_section_found = False
        for row_idx in range(1, 30):
            row = list(ws.iter_rows(min_row=row_idx, max_row=row_idx, values_only=True))[0]
            row_text = " ".join(str(cell) if cell else "" for cell in row).lower()
            
            # Skip volume sections (without %)
            if str(latest_year) in row_text and "(%" not in row_text and "volume share" not in row_text:
                volume_section_found = True
                continue
                
            # Look for percentage section after volume section
            if volume_section_found and str(latest_year) in row_text and ("(%" in row_text or "%" in row_text or "share" in row_text):
                for col_idx, cell in enumerate(row):
                    if cell and str(latest_year) in str(cell):
                        header_row_idx = row_idx
                        year_col_idx = col_idx
    ...
    if header_row_idx is None or year_col_idx is None:
        print(f"Could not find percentage data for {latest_year} in {sheet_name}")
        return [], []
    
    # Extract percentage data
    data = []
    for row_idx in range(header_row_idx + 1, min(header_row_idx + 15, ws.max_row + 1)):
        row = list(ws.iter_rows(min_row=row_idx, max_row=row_idx, values_only=True))[0]
        
        if not row[0] or "Total" in str(row[0]) or "total" in str(row[0]).lower():
            break
            
        item_name = str(row[0]).strip()
        
        # Skip header-like rows
        if item_name.lower() in ['type', 'application', 'end user', 'region', 'market breakup']:
            continue
        ...
        try:
            ...
            if 0 < value <= 1:
                # Decimal format (0.61 -> 61%)
                value = round(value * 100, 1)
            elif 1 < value <= 100:
                # Already percentage format
                value = round(value, 1)
            elif value > 100:
                # Might be raw numbers, skip or handle differently
                continue
            else:
                continue
...
def extract_dynamic_placeholders(excel_path, include_market_overview=True, include_overview_content=True):
    ...
    # Check AI content setting first
    use_ai = should_use_ai_content(excel_path)
    print(f"AI Content Setting: {'ENABLED' if use_ai else 'DISABLED'}")
    ...
    # Process all segmentation sheets
    type_top, _ = process_sheet_percentage_data_enhanced("By_Type", wb, top_n=None, latest_year=latest_year)
    app_top, app_all = process_sheet_percentage_data_enhanced("By_Application", wb, latest_year=latest_year)
    eu_top, eu_all = process_sheet_percentage_data_enhanced("By_EndUser", wb, latest_year=latest_year)
    reg_all, _ = process_sheet_percentage_data_enhanced("By_Region", wb, latest_year=latest_year)
    ...
    kv["Market_Intro_Line"] = (
        f"The {kv.get('Product', '').lower()} market in {kv.get('Country', '')} reached a volume of "
    ...
    kv["Market_Outlook_Line"] = (
        f"Overall, the {kv.get('Product', '').lower()} market in {kv.get('Country', '')} is expected to grow "
    ...
    if include_market_overview:
        kv["Market_Overview_Content"] = generate_market_overview_content(excel_path, existing_kv=kv, use_ai=use_ai)
    
    if include_overview_content:
        kv["Overview_AI_Content"] = generate_overview_ai_content(excel_path, existing_kv=kv, use_ai=use_ai)
    
    wb.close()  # Close the workbook
    return kv, volumes, use_ai

def build_report_subtitle(excel_path):
    """
    Read subtitle from Summary sheet. If not found, fallback to dynamic generation.
    """
    wb = openpyxl.load_workbook(excel_path, data_only=True)
    
    # First try to get subtitle from Summary sheet
    if "Summary" in wb.sheetnames:
        summary_ws = wb["Summary"]
        
        # Look for "Subtitle" key in column A and get its value from column B
        for row in summary_ws.iter_rows(min_row=1, max_row=summary_ws.max_row, values_only=True):
            if row[0] and str(row[0]).strip().lower() == "subtitle":
                if row[1]:  # If there's a value in column B
                    return str(row[1]).strip()
                break
    
    # Fallback to dynamic generation if no subtitle found in Summary sheet
    print("No 'Subtitle' found in Summary sheet, falling back to dynamic generation")
...
def handle_table_row_expansion_enhanced(table, template_row_idx, col_idx, template_cell, items, placeholder, excel_path):
    """
    Enhanced version that adapts to table column count and applies alternating row colors:
    - 2 columns: Item Name, 2024 Value
    - 3+ columns: Item Name, Unit, 2024 Value, 2033 Value, CAGR
    - Alternating colors: white, blue, white, blue, etc.
    """
    ...
    if items:
        ...
        # Apply alternating color for first row (template row index determines pattern)
        is_white_row = (template_row_idx % 2 == 1)  # Assuming header is row 0, data starts at row 1
        fill_table_row_with_data_and_color(table.rows[template_row_idx], col_idx, row_data, template_formatting, is_white_row, 2.0)
    ...
    if len(items) > 1:
        for i, item in enumerate(items[1:], 1):
            try:
                ...
                # Apply alternating color (template_row_idx + i determines the pattern)
                is_white_row = ((template_row_idx + i) % 2 == 1)
                fill_table_row_with_data_and_color(new_row, col_idx, row_data, template_formatting, is_white_row, 2.0)
...
def fill_table_row_with_data_and_color(row, start_col_idx, data_list, formatting, is_white_row, cell_width_inches=2.0):
    """
    Fill a table row with data, apply alternating colors, and set dynamic height
    """
    from pptx.dml.color import RGBColor
    from pptx.enum.dml import MSO_COLOR_TYPE
    from pptx.util import Inches
    
    # Extract font size from formatting for height calculation
    font_size_pt = 11  # default
    if formatting and formatting.get('font_size'):
        try:
            font_size_pt = formatting['font_size'].pt
        except:
            font_size_pt = 11
    
    # Set dynamic row height based on content
    set_dynamic_row_height(row, data_list, cell_width_inches, font_size_pt)
    ...
    for i, data in enumerate(data_list):
        ...
        if col_idx < len(row.cells):
            ...
            try:
                ...
                print(f"Warning: Could not apply background color: {e}")
            
            # Fill text content
            if cell.text_frame:
                cell.text_frame.clear()
                
                # Set consistent cell margins and enable word wrap
                ...
                try:
                    ...
                    print(f"Warning: Could not apply text color: {e}")

def calculate_dynamic_row_height(row_data, cell_width_inches=2.0, font_size_pt=11, min_height=0.35, max_height=1.0):
    """
    Calculate dynamic row height based on content length
    
    Args:
        row_data: list of strings for the row
        cell_width_inches: average cell width in inches
        font_size_pt: font size in points
        min_height: minimum row height in inches
        max_height: maximum row height in inches
    ...
    # Calculate approximate characters per line based on cell width and font size
    chars_per_inch = 72 / font_size_pt * 1.8  # Rough approximation
    chars_per_line = int(cell_width_inches * chars_per_inch)
    ...
    for data in row_data:
        ...
        lines_for_cell = max(1, (text_length + chars_per_line - 1) // chars_per_line)
        
        # Account for bullet points and special formatting
        if text.startswith("• "):
            lines_for_cell = max(1, lines_for_cell)
        
        # Track the maximum lines needed across all cells in this row
        max_lines_needed = max(max_lines_needed, lines_for_cell)
    
    # Calculate height: base height + additional height per extra line
    base_height = 0.25  # Base height for one line
    line_height = font_size_pt / 72.0 * 1.4  # Line height in inches (with spacing)
    
    calculated_height = base_height + (max_lines_needed - 1) * line_height + 0.1  # 0.1 for padding
    
    # Ensure height is within bounds
    return max(min_height, min(calculated_height, max_height))

def set_dynamic_row_height(row, row_data, cell_width_inches=2.0, font_size_pt=11):
    """
    Set dynamic height for a table row based on content
    """
    from pptx.util import Inches
    
    try:
        # Calculate dynamic height
        dynamic_height = calculate_dynamic_row_height(
            row_data, 
            cell_width_inches, 
            font_size_pt
        )
        ...
        row.height = Inches(dynamic_height)
        
        # Set consistent margins for all cells in the row
        for cell in row.cells:
            if cell.text_frame:
                ...
                cell.text_frame.margin_left = Inches(0.1)
                cell.text_frame.margin_right = Inches(0.1)
                cell.text_frame.word_wrap = True  # Enable word wrap
                
    except Exception as e:
        print(f"Warning: Could not set dynamic row height: {e}")
        # Fallback to standard height
    ...
    if use_ai and client:
        try:
            ...
            if isinstance(parsed, dict):
                details.update({k: v for k, v in parsed.items() if v})
                
        except Exception as e:
            print("AI lookup failed or timed out:", e)
    ...
    if fy_raw:
        ...
        if m:
            try:
                ...
                if 1700 <= fy_candidate <= now_year:
                    details["founding_year"] = str(fy_candidate)
                else:
                    details["founding_year"] = ""
            except:
                details["founding_year"] = ""

    return details

def distribute_company_names_across_template_slides(prs, placeholder, items, duplicate_if_needed=True, use_ai=True):
    """
    ...
    if not templates:
        print(f"⚠️ No template slides found containing {placeholder}. Nothing filled.")
        return

    total_capacity = sum(t["capacity"] for t in templates)
    print(f"Found {len(templates)} template slide(s), capacities: {[t['capacity'] for t in templates]} → total {total_capacity}")
    ...
    def _fill_table_object(tbl_obj, col_idx, header_offset, chunk_items, formatting):
        ...
        for i, company in enumerate(chunk_items):
            ...
            details = fetch_company_details(company, use_ai=use_ai)  # <-- Changed here to respect the passed use_ai
            ...
            for j, val in enumerate(values):
                try:
                    ...
                    print(f"⚠️ Error filling cell ({row_index},{col_idx + j}): {e}")
            ...
            try:
                ...
                print(f"⚠️ Error filling products cell ({row_index},{col_idx + 4}): {e}")
    ...
    # 3) handle leftovers → duplicate slides
    while i < len(items):
...
def nice_number(value):
    """
    Enhanced nice_number function for better axis scaling
    """
    if value == 0:
        return 1
...
def determine_doughnut_data_source(chart_title, slide_idx):
    """
    Determine which sheet to use for doughnut chart based on chart title or slide position.
    Returns tuple: (sheet_name, description)
    """
    if not chart_title:
        chart_title = ""
    ...
    if any(keyword in title_lower for keyword in ["physical form", "type", "by type"]):
        return "By_Type", "Physical Form"
    elif any(keyword in title_lower for keyword in ["application", "by application"]):
        return "By_Application", "Application" 
    elif any(keyword in title_lower for keyword in ["end user", "enduser", "by end user"]):
        return "By_EndUser", "End User"
    elif any(keyword in title_lower for keyword in ["region", "regional", "by region"]):
        return "By_Region", "Region"
    
    # Method 2: CORRECTED slide ranges based on your actual PPT structure
    # Section 6: Physical Form (slide ~35) 
    if 30 <= slide_idx <= 39:
        return "By_Type", "Physical Form"
    # Section 7: Application (slide ~43)
    elif 40 <= slide_idx <= 49:  
        return "By_Application", "Application"
    # Section 8: End User (slide ~50+)
    elif 50 <= slide_idx <= 59:
        return "By_EndUser", "End User"
    # Section 9: Region (slide ~60+)
    else:
        return "By_Region", "Region"

def format_column_chart_enhanced(chart, values, unit="", chart_type_name=""):
    """
    Enhanced formatting for column charts to match template style:
    - 8 Y-axis values instead of default 5
    - No gridlines
    - Data labels above bars
    - Font size 10 for axis labels
    ...
    try:
        ...
        if chart.plots and len(chart.plots) > 0:
            ...
            try:
                if hasattr(plot, 'has_major_gridlines'):
                    plot.has_major_gridlines = False
            except:
                pass
            
            # Alternative approach for gridlines
            try:
        ...
        print(f"Warning: Could not remove gridlines: {e}")
    
    try:
        # Configure Y-axis with 8 intervals and font size
        if hasattr(chart, 'value_axis') and values:
            value_axis = chart.value_axis
            
            # Calculate proper axis range
            max_value = max(values)
            min_value = 0  # Start from 0
            
            # Calculate nice round numbers for 8 intervals
            range_val = max_value - min_value
            interval = nice_number(range_val / 8)  # 8 intervals
            axis_max = math.ceil(max_value / interval) * interval
            
            # Set axis properties
            value_axis.minimum_scale = min_value
            value_axis.maximum_scale = axis_max
            value_axis.major_unit = interval
            
            # Format Y-axis labels
            value_axis.tick_labels.number_format = '0'
            if hasattr(value_axis.tick_labels, 'font'):
                value_axis.tick_labels.font.size = Pt(10)
            
            print(f"✅ Y-axis configured: 0 to {axis_max} with interval {interval}, font size 10")
            
    except Exception as e:
        print(f"Warning: Could not configure Y-axis: {e}")
    
    try:
        # Configure X-axis font size
        if hasattr(chart, 'category_axis'):
            category_axis = chart.category_axis
            if hasattr(category_axis.tick_labels, 'font'):
                category_axis.tick_labels.font.size = Pt(10)
            print("✅ X-axis labels font size set to 10")
    except Exception as e:
        print(f"Warning: Could not configure X-axis font: {e}")
    ...
    try:
        ...
        if chart.plots and len(chart.plots) > 0:
            ...
            try:
                ...
                # Set number format for data labels (no decimals for whole numbers)
                if hasattr(data_labels, 'number_format'):
                    data_labels.number_format = '0.0'  # Show one decimal place
                    
            except Exception as e:
                print(f"Warning: Could not format data labels: {e}")
                
        print("✅ Data labels added above bars")
        
    except Exception as e:
        print(f"Warning: Could not add data labels: {e}")
    
    try:
        # Style the chart series (bars) with different colors for each bar
        if chart.series and len(chart.series) > 0:
            series = chart.series[0]
            
            # Define a color palette with distinct colors for each bar
            color_palette = [
                ...
                RGBColor(255, 102, 0),     # Bright Orange
            ...
            try:
                ...
                print(f"✅ Applied different colors to {len(series.points)} bars")
            except Exception as e:
                print(f"Warning: Could not apply colors to bars: {e}")
                
    except Exception as e:
        print(f"Warning: Could not style chart series: {e}")

def update_charts_in_slide_enhanced_fixed(
    slide,
    slide_idx,
    volumes,
    unit,
    historical_years=[2019, 2020, 2021, 2022, 2023, 2024],
    forecast_years=[2025, 2026, 2027, 2028, 2029, 2030, 2031, 2032, 2033],
    excel_path=None
):
    """
    Updated version with enhanced column chart formatting
    """
    charts_to_recreate = []

    # ---------- First pass: identify charts and their properties ----------
    for shape in slide.shapes:
        ...
        title_text = ""
        try:
            if chart.has_title and chart.chart_title and getattr(chart.chart_title, "text", None):
                title_text = chart.chart_title.text
        except Exception:
            pass

        # ---------------- Handle COLUMN charts (ENHANCED) ----------------
        if chart.chart_type == XL_CHART_TYPE.COLUMN_CLUSTERED:
            is_historical = False
            is_forecast = False

            title_lower = (title_text or "").lower()
            if "performance" in title_lower or "historical" in title_lower:
                is_historical = True
            elif any(k in title_lower for k in ("forecast", "outlook", "market forecast")):
                is_forecast = True
            else:
                # Check categories for year ranges
                try:
                    existing_categories = []
                    if chart.plots and len(chart.plots) > 0:
                        plot = chart.plots[0]
                        if hasattr(plot, "categories") and plot.categories:
                            existing_categories = [str(cat) for cat in plot.categories if cat]

                    if any(str(year) in str(cat) for cat in existing_categories for year in historical_years):
                        is_historical = True
                    elif any(str(year) in str(cat) for cat in existing_categories for year in forecast_years):
                        is_forecast = True
                except Exception:
                    pass

            if not (is_historical or is_forecast):
                print(f"Could not determine chart type for title: '{title_text}'")
                continue

            years = historical_years if is_historical else forecast_years
            chart_type_name = "historical" if is_historical else "forecast"
            ...
            charts_to_recreate.append(chart_info)
            print(f"Identified {chart_type_name} chart for recreation: {title_text}")

        # ---------------- Handle DOUGHNUT charts (unchanged) ----------------
        if chart.chart_type == XL_CHART_TYPE.DOUGHNUT:
            if not excel_path:
                print("Skipping doughnut chart: excel_path not provided")
                continue

            # Determine data source based on title or slide position
            sheet_name, description = determine_doughnut_data_source(title_text, slide_idx)
            
            print(f"Slide {slide_idx + 1} - Doughnut chart detected - Title: '{title_text}' -> Using {sheet_name} data ({description})")

            # Load workbook for percentage data
            try:
                wb = openpyxl.load_workbook(excel_path, data_only=True)
            except Exception as e:
                print(f"Failed to load workbook for doughnut data: {e}")
                continue

            # Get percentage data from the determined sheet using ENHANCED function
            latest_year = 2024
            try:
                # Use the enhanced function for better data extraction
                percentage_data, _ = process_sheet_percentage_data_enhanced(sheet_name, wb, latest_year=latest_year)
                
            except Exception as e:
                print(f"Failed to process percentage sheet {sheet_name} for doughnut: {e}")
                continue

            if not percentage_data:
                print(f"No percentage data found for doughnut chart from {sheet_name}")
                continue
            ...
            charts_to_recreate.append(chart_info)
            print(f"Queued doughnut chart for recreation with {sheet_name} data")

    # ---------- Second pass: recreate charts ----------
    for chart_info in charts_to_recreate:
        try:
            # ================= Recreate COLUMN charts (ENHANCED) =================
            if chart_info.get("chart_kind") == "column":
                volume_values = [float(volumes.get(year, 0)) for year in chart_info["years"]]
                print(f"Recreating {chart_info['chart_type_name']} chart with data: {dict(zip(chart_info['years'], volume_values))}")

                # Remove old shape
                old_shape = chart_info["shape"]
                shape_element = old_shape.element
                slide.shapes._spTree.remove(shape_element)

                # Build chart data
                chart_data = CategoryChartData()
                chart_data.categories = [str(year) for year in chart_info["years"]]
                chart_data.add_series("", tuple(volume_values))
                ...
                format_column_chart_enhanced(new_chart, volume_values, unit, chart_info["chart_type_name"])

                # Title + placeholders
                try:
                    if chart_info.get("title"):
                        updated_title = chart_info["title"]
                        updated_title = updated_title.replace("{{Unit}}", str(unit))
                        updated_title = updated_title.replace("{{Historical_Start_Year}}", str(min(historical_years)))
                        updated_title = updated_title.replace("{{Historical_End_Year}}", str(max(historical_years)))
                        updated_title = updated_title.replace("{{Forecast_Start_Year}}", str(min(forecast_years)))
                        updated_title = updated_title.replace("{{Forecast_End_Year}}", str(max(forecast_years)))
                        new_chart.chart_title.text = updated_title
                        print(f"Set chart title: {updated_title}")
                except Exception as e:
                    print(f"Warning: could not set column chart title: {e}")

                print(f"✅ Successfully recreated {chart_info['chart_type_name']} chart with enhanced formatting")

            # ================= DOUGHNUT CHART (unchanged) =================
            elif chart_info.get("chart_kind") == "doughnut":
                # [Doughnut chart code remains the same as in your original script]
                percentage_data = chart_info["data"]
                labels = [name for name, _ in percentage_data]
                values = [float(val) for _, val in percentage_data]

                print(f"Creating doughnut chart from {chart_info['sheet_name']} ({chart_info['description']}) with data: {dict(percentage_data)}")
                
                # Store position info
                left = chart_info["left"]
                top = chart_info["top"] 
                ...
                try:
                    ...
                    # Apply all the formatting (legend, hole size, data labels, colors, etc.)
                    # 1. Legend at bottom
                    try:
                        ...
                        # Set legend font size to 10
                        if new_chart.legend.font:
                            new_chart.legend.font.size = Pt(10)
                        
                        print("✅ Legend positioned at bottom with proper spacing and font size 10")
                    except Exception as e:
                        print(f"Warning: Legend failed: {e}")
                    
                    # 2. Doughnut hole size
                    try:
                        if new_chart.plots and len(new_chart.plots) > 0:
                            plot = new_chart.plots[0]
                            if hasattr(plot, 'doughnut_hole_size'):
                                plot.doughnut_hole_size = 60
                                print("✅ Doughnut hole configured")
                    except Exception as e:
                        print(f"Warning: Hole size failed: {e}")
                    
                    # 3. Percentage data labels
                    try:
                        if new_chart.plots and len(new_chart.plots) > 0:
                            plot = new_chart.plots[0]
                            plot.has_data_labels = True
                            ...
                            # Turn off automatic percentage display
                            data_labels.show_value = False
                            data_labels.show_percentage = False
                            data_labels.show_category_name = False
                            data_labels.show_series_name = False
                            
                            # Set data labels font size to 10
                            if data_labels.font:
                                data_labels.font.size = Pt(10)
                            
                            # Manually set each data label with exact decimal formatting
                            if new_chart.series and len(new_chart.series) > 0:
                                series = new_chart.series[0]
                                
                                for i, (label, value) in enumerate(percentage_data):
                                    if i < len(series.points):
                                        point = series.points[i]
                                        if hasattr(point, 'data_label'):
                                            point.data_label.has_text_frame = True
                                            point.data_label.text_frame.text = f"{value:.1f}%"
                                            
                                            # Set font size for individual data label
                                            if point.data_label.text_frame.paragraphs:
                                                para = point.data_label.text_frame.paragraphs[0]
                                                if para.runs:
                                                    para.runs[0].font.size = Pt(10)
                            
                            print("✅ Custom percentage labels configured with exact decimal values and font size 10")
                    except Exception as e:
                        print(f"Warning: Custom data labels failed: {e}")
                    
                    # 4. Colors (vary by data source for visual distinction)
                    try:
                        # Different color schemes for different data sources
                        color_schemes = {
                            "By_Type": [
                                RGBColor(68, 114, 196),   # Blue
                                RGBColor(112, 173, 71),   # Green
                        ...
                        colors = color_schemes.get(chart_info['sheet_name'], color_schemes["By_Type"])
                        ...
                        if new_chart.series and len(new_chart.series) > 0:
                            ...
                            print(f"✅ Colors applied for {chart_info['sheet_name']} data")
                    except Exception as e:
                        print(f"Warning: Segment colors failed: {e}")
                    
                    # 5. Title with data source indication
                    try:
                        if chart_info.get("title"):
                            updated_title = chart_info["title"].replace("{{Latest_Year}}", str(chart_info["latest_year"]))
                            new_chart.chart_title.text = updated_title
                            print(f"✅ Title set for {chart_info['description']}: {updated_title}")
                    except Exception as e:
                        print(f"Warning: Title failed: {e}")
                    
                    print(f"✅ Doughnut chart with {chart_info['sheet_name']} data created successfully")
                    
                except Exception as e:
                    print(f"❌ Doughnut chart creation failed: {e}")
                    continue

        except Exception as e:
            print(f"❌ Failed to recreate chart: {e}")

# Alternative simpler approach - just for testing
def simple_chart_update_test(slide, volumes, unit):
    """
    Simplified test function to verify chart updating works at all.
    """
    print("=== SIMPLE CHART UPDATE TEST ===")
    ...
    for i, shape in enumerate(slide.shapes):
        ...
        chart = shape.chart
        print(f"Chart {i}: Type = {chart.chart_type}")
        
        if chart.chart_type == XL_CHART_TYPE.COLUMN_CLUSTERED:
            try:
                # Get some test data
                test_years = [2019, 2020, 2021, 2022, 2023, 2024]
                test_values = [volumes.get(year, year * 100) for year in test_years]  # Use year*100 as fallback
                
                print(f"Test data: {dict(zip(test_years, test_values))}")
                
                # Try the most basic approach
                chart_data = CategoryChartData()
                chart_data.categories = [str(year) for year in test_years]
                chart_data.add_series('Test Series', tuple(test_values))
                
                chart.replace_data(chart_data)
                print(f"✅ Basic update attempted for chart {i}")
                
            except Exception as e:
                print(f"❌ Basic update failed for chart {i}: {e}")

# Add this debug function to your main() function - call it before the normal chart update
def debug_and_fix_charts(prs, volumes, unit):
    """
    Debug function to test different chart update approaches
    """
    print("\n" + "="*60)
    print("DEBUGGING CHART UPDATES")
    print("="*60)
    
    for slide_idx, slide in enumerate(prs.slides):
        has_charts = any(shape.has_chart for shape in slide.shapes)
        if has_charts:
            print(f"\nSlide {slide_idx + 1} has charts - testing updates...")
            simple_chart_update_test(slide, volumes, unit)
...
def main(excel_file, ppt_template, output_ppt, callback=None):
    ...
    try:
        ...
        update_step_progress(2, 'active', 'Loading Excel workbook...')
        
        kv = read_summary_keys(excel_file, "Summary")
        update_step_progress(2, 'active', 'Extracting dynamic placeholders...')
        
        dynamic_kv, volumes, use_ai = extract_dynamic_placeholders(excel_file, include_market_overview=True, include_overview_content=True)
        kv.update(dynamic_kv)
        kv["Subtitle"] = build_report_subtitle(excel_file)
        
        update_step_progress(2, 'completed', 'Excel data loaded successfully')

        # Step 3: Processing data sheets
        update_step_progress(3, 'active', 'Creating inline placeholders...')
        ...
        update_step_progress(3, 'active', 'Processing segmentation sheets...')
        ...
        update_step_progress(3, 'completed', 'Data processing complete')

        # Step 4: AI Content (if enabled)
        update_step_progress(4, 'active', 'Processing AI content...')
        # AI processing happens within the functions, so just mark as completed
        update_step_progress(4, 'completed', 'AI content processing complete')

        # Step 5: Loading PowerPoint template
        update_step_progress(5, 'active', 'Opening PowerPoint template...')
        prs = Presentation(ppt_template)
        
        update_step_progress(5, 'active', 'Building table of contents...')
        toc_items = build_toc_from_sheet(excel_file, "Table_Contents")
        handle_toc_multi_slides(prs, toc_items)
        
        update_step_progress(5, 'completed', 'Template loaded successfully')

        # Step 6: Replacing placeholders
        update_step_progress(6, 'active', 'Analyzing volume data...')
        
        # Debug: Print volumes data
        print("\n" + "="*50)
        print("VOLUME DATA FROM EXCEL:")
        print("="*50)
        for year in sorted(volumes.keys()):
            print(f"  {year}: {volumes[year]:,.0f}")
        print("="*50)

        update_step_progress(6, 'active', 'Processing slide content...')
        ...
        for slide_idx, slide in enumerate(prs.slides):
            ...
            # Update progress for every few slides
            if slide_idx % 3 == 0:  # Update every 3 slides
                update_step_progress(6, 'active', f'Processing slide {slide_idx + 1} of {total_slides}...')
            ...
            # ENHANCED CHART UPDATES WITH SLIDE INDEX
            historical_years = list(range(2019, 2025))  # 2019-2024
            forecast_years = list(range(2025, 2034))    # 2025-2033
            
            # Check if this slide has charts
            has_charts = any(shape.has_chart for shape in slide.shapes)
            if has_charts:
                print(f"🔄 Recreating charts on slide {slide_idx + 1}...")
                update_charts_in_slide_enhanced_fixed(
                    slide, 
                    slide_idx,  # Pass slide index
                    volumes, 
                    kv.get("Unit", ""), 
                    historical_years, 
                    forecast_years, 
        ...
        update_step_progress(6, 'completed', 'Placeholders updated successfully')

        # Step 7: Processing tables and company data
        update_step_progress(7, 'active', 'Processing company tables...')
        
        # Company table placeholders
        company_items = build_list_from_sheet(excel_file, "Company_Name")
        distribute_company_names_across_template_slides(prs, "{{Company_Name_List}}", company_items, duplicate_if_needed=True, use_ai=use_ai)
        
        update_step_progress(7, 'completed', 'Charts and tables updated')

        # Step 8: Saving presentation
        update_step_progress(8, 'active', 'Creating backup...')
        
        # Save with a backup
        backup_file = output_ppt.replace('.pptx', '_backup.pptx')
        try:
